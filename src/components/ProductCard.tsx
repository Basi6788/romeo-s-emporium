...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = () => {
      // Scale thoda badhaya hai taake hover pe smooth zoom effect aur enhance ho
        ...
        >
          ...
          <div className="relative aspect-square bg-muted/50 overflow-hidden">
            <img
              ...
              src={product.image}
              alt={product.name}
              // Changed: object-contain -> object-cover, removed p-6
              className="w-full h-full object-cover transition-transform will-change-transform"
            />
            
            {/* Discount Badge */}
            {discount > 0 && (
              <div className="absolute top-3 left-3 px-2.5 py-1 rounded-lg bg-rose-500 text-white text-xs font-bold z-10">
                -{discount}%
              </div>
            )}
            ...
            <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
              ...
              {/* Optional: Small eye icon can stay, or remove if you want NO quickview at all */}
              <button
            ...
            {/* REMOVED: Bottom Quick View Button was here */}
          </div>
