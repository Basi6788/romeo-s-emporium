...
const Header = () => {
  ...
  // --- COLORS: ACTIVE STATE FIXED (Solid Color on Click) ---
  // "active:bg-black" aur "active:bg-white" bina opacity ke use kiye hain
  ...
  const dropdownBgClass = isHeaderDark 
    ? 'bg-[#0a0a0a] text-white border-white/10 shadow-white/5' 
  ...
  const bubbleClass = `
    ...
    backdrop-blur-md floating-shadow border transition-all duration-200 active:scale-95
  ...
  return (
    <>
      ...
      {/* --- FLOATING HEADER (Changed 'fixed' to 'sticky' to fix scrolling issue) --- */}
      {/* 'sticky' parent transform issues ko bypass karke top par chipka rehta hai */}
      <header className="sticky top-2 left-0 right-0 z-[100] px-3 w-full pointer-events-none mb-[-60px]">
        <div className="flex items-center justify-between w-full max-w-7xl mx-auto">
          ...
          <div className="pointer-events-auto relative">
            ...
            >
              <img src={user?.imageUrl || "https://img.clerk.com/preview.png"} alt="User" className="w-full h-full object-cover rounded-full" />
            </button>
            {avatarMenuOpen && (
          ...
          <div className="pointer-events-auto flex-1 max-w-md mx-3 relative group hover:scale-[1.02] transition-transform duration-300">
             ...
             <div className={`relative h-12 rounded-full px-5 flex items-center justify-start z-10 w-full transition-colors duration-200 ${headerBgClass}`}>
          ...
          <div className="pointer-events-auto flex items-center gap-3">
             ...
             <button onClick={() => setMenuOpen(!menuOpen)} className={`${bubbleClass} w-12 h-12 rounded-full relative overflow-hidden hover:scale-105`}>
          ...
          <div className="relative z-[100] w-full max-w-sm px-6 flex flex-col gap-3 pointer-events-auto">
             {menuItems.map((item, idx) => (
               <button 
                 ...
                 className={`nav-pill w-full py-4 rounded-full text-lg font-bold tracking-wide border shadow-2xl gpu-accelerated flex items-center justify-center gap-3 relative transition-all duration-200 active:scale-95 ${headerBgClass} ${hoverClass} ${isHeaderDark ? 'border-white/10' : 'border-black/10'}`}
               >
                 {item.label}
             ...
             <button 
               ...
               className={`nav-pill mt-4 w-full py-3.5 rounded-full flex items-center justify-center gap-3 border shadow-xl gpu-accelerated transition-all duration-200 active:scale-95 ${headerBgClass} ${hoverClass} ${isHeaderDark ? 'border-white/10' : 'border-black/10'}`}
             >
